function [datapoints1, datapoints2, datapoints3, fit1, fit2, fit3, value_coherence, value_coherence2, value_coherence3, value_coherence_rounded, value_coherence_rounded2, value_coherence_rounded3] = FitThreshold(dataset, fitting, bootstrapping)%% fit a psychometric function
% Note: O. Colizoli, May 2015. I replaced the datasample function with
% datasample2, because this function was missing on the mac mini in lab
% 1.01. datasample2 is now saved in the same folder with the 2AFC scripts.
    ...
    case 'bootstrap',
        ...
        for iboot = 1:nboot,
            ...
            for idiff = 1:length(datapoints1.mean.difficulty),
                ...
                [bootstrappeddata] = datasample2(datapoints1.all.correct(datapoints1.all.difficulty==datapoints1.mean.difficulty(idiff)), ...
...
switch fitting,
    case 'fit',
        ...
        n2 = 0.85;
        n3 = 0.65;        
        [d,idx] = min(abs(fit1.y-n));
        [d,idx2] = min(abs(fit1.y-n2));
        [d,idx3] = min(abs(fit1.y-n3));        
%         value = fit1.y(idx);
        value_coherence = fit1.x(idx);
        value_coherence2 = fit1.x(idx2);
        value_coherence3 = fit1.x(idx3);
        value_coherence_rounded = round(value_coherence,2);        
        value_coherence_rounded2 = round(value_coherence2,2);
        value_coherence_rounded3 = round(value_coherence3,2);
...
datapoints2.all.response(datapoints2.all.response == 0) = 0;
datapoints2.all.increment(datapoints2.all.increment == 0) = 0;
datapoints2.all.response(datapoints2.all.response == 180) = 1;
datapoints2.all.increment(datapoints2.all.increment == 180) = 1;
    ...
    case 'bootstrap',
        ...
        for iboot = 1:nboot,
            ...
            for idiff = 1:length(datapoints1.mean.difficulty),
                ...
                % get a random sample from all the datapoints with replacement - only for this level of difficulty
                sampleidx = datasample2(thisdiffidx, length(thisdiffidx)); %), 'Replace', true);
    ...
    case 'bootstrap',
        ...
        for iboot = 1:nboot,
            ...
            for icoh = 1:length(datapoints3.mean.coherence),
                ...
                [bootstrappeddata] = datasample2(datapoints3.all.response(datapoints2.all.coherence==datapoints3.mean.coherence(icoh)), ...
...
xlabel('<- right    | coherence (%) |    left ->'); ylabel('% responses LEFT');
...
figure_name = sprintf('Figure%d_threshold', setup.participant);
figure_name2 = ['Data/P' num2str(setup.participant) '_'  setup.datetime '_Threshold' '/' figure_name];
saveas(gcf, figure_name2, 'pdf');
